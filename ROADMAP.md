...
- generated values, observed resource state and references to secrets;
...
2. Define transport- and persistence-independent records for Application, Component, Resource, Binding, Configuration document and authenticated workload identity.
...
6. Keep the existing API and configurator behind compatibility adapters until the new model has focused replacement and reconciliation tests.
...
> Continue the Haby OSS revival using `ROADMAP.md`, `docs/architecture/application-manifest.md` and `docs/architecture/workload-identity.md`. Implement only the **Next architecture work package**. Start with the domain terminology ADR and side-effect-free manifest and workload-identity contracts, schema, parser and contract tests. Do not add the complete delivery endpoint or publisher implementations, replace persistence, execute plugin side effects, migrate the UI or add remote providers in the same change. Preserve the legacy configurator behind compatibility adapters, run focused tests and `git diff --check`, and report environmental blockers separately from code failures.
